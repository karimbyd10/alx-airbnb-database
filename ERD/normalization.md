...
### Observed Issues
1. **Bookings.total_price** depends on other table data (property price and booking duration), causing a transitive dependency.
2. Address in `Properties` may contain multiple data points (street, city, state, country) → violates atomicity.

## Step 2: Apply 3NF Principles
- Remove derived data (`Bookings.total_price`).
- Split `Properties.address` into atomic fields: street, city, state, country.
- Maintain foreign key relationships for referential integrity.
